> [正体中文](./build-first-agent-in-7-steps.md) | **简体中文** | [English](./build-first-agent-in-7-steps.en.md)

# 7 步打造你的第一个 AI Agent

> [← 回主路线 README](../README.zh-CN.md)

> 📌 **这份是给 Track B（Agent Builder）的**——教你**从零写**一个 agent。
> 走 [Track A（CLI Power User）](../tracks/cli/A1-cli-intro.zh-CN.md) 的人**不需要跑**这份；但读过之后对「**agent 从 LLM API 到 production 怎么一步步组起来**」会有更深的理解，可作为 optional 进阶补充。
...
验证它是否生效：在 Claude Code 对话里贴 `https://arxiv.org/abs/2210.03629`，看 Claude 是不是按你定义的格式回应。
...
**进阶**：如果想把这个 skill 包成可分享的 plugin（让别人也能装在自己的 Claude Code），参考 [Stage 5.4 Plugins & Marketplaces](../stages/05-claude-code-ecosystem.zh-CN.md#54--plugins-与-marketplaces)。本 walkthrough 不展开 plugin 打包流程。
